...
Answer:"""
    ...
    def get_txt_history():
        if "chat_history" in st.session_state and st.session_state.chat_history:
            return "\n\n".join(
                f"Q{i + 1}: {q}\nA{i + 1}: {a}" for i, (q, a) in enumerate(st.session_state.chat_history)) #get the history and add 1 to the position in the array for the question number
    ...
    user_question = st.text_input("", placeholder="e.g. How did home run rates change over time?") #placeholder to give user an idea of what to say
